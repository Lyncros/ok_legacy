...
class TiporelacionBSN extends BSN {
    ...
    public function comboRelacionStr($valor = 0, $tiporel = 'UP', $campo = "id_tiporel", $class = "cd_celda_input") {
        $strC='';
        if (in_array($tiporel, $this->arrayTipos)) {
            $tipoRel = $this->coleccionRelacionByTipo($tiporel);
            $strC.= ("<select name='" . $campo . "' id='" . $campo . "' class='campos_btn' >\n");
            for ($pos = 0; $pos < sizeof($tipoRel); $pos++) {
                $strC.=("<option value='" . $tipoRel[$pos]['id_tiporel'] . "'");
                if ($tipoRel[$pos]['id_tiporel'] == $valor) {
                    $strC.= (" SELECTED ");
                }
                $strC.=(">" . $tipoRel[$pos]['relacion'] . " - " . $tipoRel[$pos]['grado'] . "</option>\n");
            }
            $strC.= "</select>\n";
        }
        return $strC;
...
//			$tipoDB = new TiporelacionPGDAO($arrayTabla);
    ...
    public function armaListaRelacionesByTipo($tipo) {
        $retorno = '';
        $arrayRel = $this->coleccionRelacionByTipo($tipo);
        if (sizeof($arrayRel) != 0) {
            foreach ($arrayRel as $registro) {
                $retorno.=($registro['id_tiporel'] . ',');
}
    ...
    public function getJsonTipoUC() {
        $arrayRel = $this->coleccionRelacionByTipo('UC');
        $arrRet=array();
        foreach ($arrayRel as $rela) {
            $arrRet[]=array( $rela['id_tiporel'], $rela['relacion'] . " " . $rela['grado'] );
        }
