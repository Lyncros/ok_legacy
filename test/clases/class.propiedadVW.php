...
//        $ubiBSN = new UbicacionpropiedadBSN();
    ...
    public function vistaDatosPropiedadBuscador($id_cli = 0) {
        ...
        print "                                                    </tr>\n";
        print "                                                    <tr>\n";
        print "                                                        <td colspan=\"2\" style=\"font-size:10pt; color:#646419;\">" . $this->arrayForm['calle'] . " " . $this->arrayForm['nro'] . " " . $this->arrayForm['piso'] . " " . $this->arrayForm['dpto'] . "</td>\n";
        print "                                                    </tr>\n";
        print "                                                </table></td>\n";
        print "                                        </tr>\n";
        print "                                        <tr>\n";
...
//***********************************************************************************************************************************
    ...
    public function vistaTablaPropiedadORIG($pagina = 1, $idcrm = '') {
        $ubiBSN = new UbicacionpropiedadBSN();
...
//***********************************************************************************************************************************
    ...
    public function vistaToolsPropiedad() {
        $perf = new PerfilesBSN();
        ...
        $arrayTools[] = array('Volver', 'images/user--arrow.png', 'menuTools(17)');
        $menu = new Menu();
        $strMenu = $menu->barraHerramientas2String($arrayTools);
        return $strMenu;
    }

    public function vistaToolsSeleccion() {
        $strMenu = '';
        $strMenu.= "	<div id='seleccion' style='float: right; width: 250px;padding-top:5px;'>\n";
        $strMenu.="		<span class='row' align='left' style='padding-right:3px; font-weight: bold;font-size:14px;'>Selecci&oacute;n&nbsp;</span>\n";
        $strMenu.="		<img src='images/trash.png' onclick='javascript:limpiarAdjuntos();' title='Limpiar Selecci&oacute;n' style='vertical-align: middle; padding: 2px; cursor: pointer;'>&nbsp;\n";
        $strMenu.="     <img src='images/basket_edit.png' onclick='javascript:verSeleccion();' title='Ver Selecci&oacute;n' style='vertical-align: middle; padding: 2px; cursor: pointer;'>&nbsp;\n";
        $strMenu.="     <img src='images/email.png' onclick='javascript:enviarSeleccion();' title='Enviar Selecci&oacute;n' style='vertical-align: middle; padding: 2px; cursor: pointer;'>&nbsp;<span id='cuantos'></span>\n";
        $strMenu.="     <img src='images/eye.png' onclick='javascript:verLista();' title='Ver Lista' style='vertical-align: middle; padding: 2px; cursor: pointer;'>\n";
        $strMenu.="	</div>\n";
        return $strMenu;
    }

//    public function filtraListaPropiedades($pagina = 1, $idcrm = '') {
    public function filtraListaPropiedades($pagina = 1) {
        //[buscaProp] => [aux_prop] => [aux_Tprop] => 9 [aux_selTprop] => Casas [aux_operacion] => "Venta" 
        //[aux_seloperacion] => Venta [ID_UBICAPRINCIPAL] => 3979 [fid_ubica] => 4040,4076,4069 
        //[aux_emp] => 152,154,185 [aux_selemp] => ALTOS DEL PARQUE II, ALTOS DEL PARQUE I, ALTOS DEL PARQUE III 
        //[DORMITORIOS] => 2 [opcion] => [id_cli] => 20130521201147 [activa] => [cpo] => [fid_tipo_prop] => 9 
        //[fid_seltipo_prop] => Casas [fid_codigo] => 0 [fid_calle] => [fid_selloca] => [fid_emp] => 152,154,185 
        //[fid_selemp] => ALTOS DEL PARQUE II, ALTOS DEL PARQUE I, ALTOS DEL PARQUE III [foperacion] => "Venta" 
        //[seloperacion] => Venta        
        $ubiBSN = new UbicacionpropiedadBSN();
        $sucursal = new Sucursal();
        $perf = new PerfilesBSN();
...
//        if (isset($_SESSION['filtro']['adjuntos']) && $_SESSION['filtro']['adjuntos'] != '') {
//            $aux_adjuntos = $_SESSION['filtro']['adjuntos'];
        if (isset($_SESSION['adjuntos']) && $_SESSION['adjuntos'] != '') {
//            $aux_adjuntos = $_SESSION['adjuntos'];
//            $sel = split(',', $aux_adjuntos);
            $sel = $_SESSION['adjuntos'];
            $cantSel = "(" . count($sel) . ")";
...
//        $dpropVW->armaFiltroDatosprop2Txt($arraydp, $_POST['aux_Tprop']); // Carga nuevamente el filtro con los valores leidos
        $dpropBSN = new DatospropBSN();
        $arrayDB = $arraydp;
        ...
        if (array_key_exists('vistasel', $_POST) && $_POST['vistasel'] != '1') {
            ...
            // Cambio para el manejo de los adjuntos como id:estado
            $adjuntosPuros = '';
            $propsIN = implode(",", array_keys($sel));
        }
        $aux_campo = '';
        ...
        //[fid_selemp] => ALTOS DEL PARQUE II, ALTOS DEL PARQUE I, ALTOS DEL PARQUE III [foperacion] => "Venta" 
        //[seloperacion] => Venta        
        $aux_codigo = $_POST['fid_codigo'];
        $aux_calle = $_POST['fid_calle'];
        $idsZonas = $_POST['fid_ubica'];
        $aux_prop = $_POST['aux_Tprop'];
        $aux_operacion = $_POST['aux_operacion'];
        $aux_emp = $_POST['aux_emp'];
        $propsIN=$_POST['aux_prop'];
        //$pagina=$_POST[''];
        $aux_orden = '';
        $aux_vistaestado = 0;
        $aux_vistazona = '';
        $arrayDatos = $evenBSN->cargaColeccionFiltroBuscadorMapa($aux_codigo, $aux_calle, $idsZonas, $aux_prop, $aux_operacion, $aux_emp, $propsIN, $pagina, $aux_campo, $aux_orden, $aux_vistaestado, $aux_vistazona);
        $totalregs = $evenBSN->cantidadRegistrosFiltroBuscador($aux_codigo, $aux_calle, $idsZonas, $aux_prop, $aux_operacion, $aux_emp, $propsIN, $aux_vistaestado);
        return array($arrayDatos,$totalregs);
    }
...
//                print $Even['id_sucursal'] . str_repeat("0", 5 - strlen(strval($Even['id_prop']))) . $Even['id_prop'];
    ...
    public function deslpiegaTabla2String($arrayDatos, $registros, $totalregs, $pagina, $sel) {
        $strTabla = '';
        $perfGpo = $_SESSION['perfGpo'];
        $perfSuc = $_SESSION['perfSuc'];
        $listaTPG = $_SESSION['listaTPG'];
        ...
        $strTabla.= "<div id='vistaTabla' class='vistaTabla' >\n";  // DIV general de la tabla
//        $strTabla.= "    <ul>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propAcc'>&nbsp;</td>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propCod'  onclick='javascript:ordenar(1);'>C&oacute;digo<img src='images/" . $img1 . "' id='img_1' border='0' /></li>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propZona' onclick='javascript:ordenar(2);'>Zona<img src='images/" . $img2 . "' id='img_2' border='0' /></li>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propDir'  onclick='javascript:ordenar(4);'>Direcci&oacute;n<img src='images/" . $img4 . "' id='img_4' border='0' /></li>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propTipo' onclick='javascript:ordenar(5);'>Tipo<img src='images/" . $img5 . "' id='img_5' border='0' /></li>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propSup'  onclick='javascript:ordenar(6);'>Sup.<img src='images/" . $img6 . "' id='img_6' border='0' /></li>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propAmb'  onclick='javascript:ordenar(7);'>Amb.<img src='images/" . $img7 . "' id='img_7' border='0' /></li>\n";
//        $strTabla.= "     <li class='li_lista_titulo' id='propVal'  onclick='javascript:ordenar(9);'>Valor<img src='images/" . $img9 . "' id='img_9' border='0' /></li>\n";
//        $strTabla.= "	  </ul>\n";
        $strTabla.="<div width=\"100%\">";
        $strTabla.= "     <div class='div_lista_titulo' id='propAcc'>$totalregs&nbsp;</div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propCod' title='Ordenar por código' onclick='javascript:ordenar(1);'>C&oacute;digo<img src='images/" . $img1 . "' id='img_1' border='0' /></div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propZona' title='Ordenar por zona' onclick='javascript:ordenar(2);'>Zona<img src='images/" . $img2 . "' id='img_2' border='0' /></div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propDir' title='Ordenar por dirección'  onclick='javascript:ordenar(4);'>Direcci&oacute;n<img src='images/" . $img4 . "' id='img_4' border='0' /></div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propTipo' title='Ordenar por tipo propiedad' onclick='javascript:ordenar(5);'>Tipo<img src='images/" . $img5 . "' id='img_5' border='0' /></div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propSup' title='Ordenar por superficie'  onclick='javascript:ordenar(6);' style='text-align: center;'>Sup<img src='images/" . $img6 . "' id='img_6' border='0' /></div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propAmb' title='Ordenar por ambientes'  onclick='javascript:ordenar(7);' style='text-align: center;'>Amb<img src='images/" . $img7 . "' id='img_7' border='0' /></div>\n";
        $strTabla.= "     <div class='div_lista_titulo' id='propVal' title='Ordenar por valor'  onclick='javascript:ordenar(9);' style='text-align: center;'>Valor<img src='images/" . $img9 . "' id='img_9' border='0' /></div>\n";
        $strTabla.= "<div id=\"clearfix\"></div>\n";
        $strTabla.="</div>";
...
//        $arrayEven = array_slice($arrayDatos, (($pagina - 1) * $registros), $registros);   // AGREGADO PARA UNIFICAR BUSQUEDA
        $arrayEven = $arrayDatos;
        $arrayZonas = array();
        $strTabla.= "<div style='overflow:auto; clear:both; height:600px; width:795px;'>\n";
        if (sizeof($arrayEven) == 0) {
            $strTabla.= "No existen datos para mostrar";
        } else {
            $zonaAnt = 0;

            $arrTPop[1] = "<img src='images/tipo_depto.png' height='16' border='0' title='Departamento' />";
            $arrTPop[9] = "<img src='images/tipo_casa.png' height='16' border='0' title='Casa' />";
            $arrTPop[2] = "<img src='images/tipo_local.png' height='16' border='0' title='Local' />";
            $arrTPop[3] = "<img src='images/tipo_ph.png' height='16' border='0' title='PH' />";
            $arrTPop[6] = "<img src='images/tipo_terreno.png' height='16' border='0' title='Terreno' />";
            $arrTPop[7] = "<img src='images/tipo_terreno.png' height='16' border='0' title='' />";
            $arrTPop[11] = "<img src='images/tipo_oficina.png' height='16' border='0' title='Oficina' />";
            $arrTPop[15] = "<img src='images/tipo_galpon.png' height='16' border='0' title='Galp&oacute;n' />";
            $arrTPop[16] = "<img src='images/tipo_terreno.png' height='16' border='0' title='Chacra' />";
            $arrTPop[17] = "<img src='images/tipo_terreno.png' height='16' border='0' title='Quinta' />";
            $arrTPop[18] = "<img src='images/tipo_galpon.png' height='16' border='0' title='Cochera' />";
            $arrTPop[19] = "<img src='images/tipo_galpon.png' height='16' border='0' title='Industrial' />";
...
//                if ($zonaAnt != $Even['id_ubica']) {
                    if (array_key_exists($Even['id_ubica'], $arrayZonas)) {
                        $txtZona = $arrayZonas[$Even['id_ubica']];
                    } else {
                        $txtZona = $ubi1BSN->armaNombreZonaAbr($Even['id_ubica']);
//                        $zonaAnt = $Even['id_ubica'];
                        $arrayZonas[$Even['id_ubica']] = $txtZona;
                    }
//                }
                // Fin de definicion de informacion
                $fila=0;    
                ...
                $strTabla.= "<div class=\"div_lista_" . $fila . "\">";
                $strTabla.= "	 <div id='propAcc'>";

//                if (in_array($Even['id_prop'], $sel)) {
//                $idPos=array_search($Even['id_prop'], $selId);
//                if ($idPos!==FALSE) {
                $claves=  array_keys($selEst);
//                if (array_key_exists(strval($Even['id_prop']), $selEst)) {
                if (in_array($Even['id_prop'], $claves)) {
                    switch ($selEst[$Even['id_prop']]) {
                        case 1:
                            $basket = "basket_delete.png";
                            break;
                ...
                $strTabla.= "&nbsp;<img src='images/" . $basket . "' id='seleccionar" . $Even['id_prop'] . "' title='Seleccionar' border='0' style='cursor: pointer;' onclick=\"javascript: agregarSeleccion(" . $Even['id_prop'] . ");\">";
                if ($listaTPG == 0 || strpos($listaTPG, $Even['id_tipo_prop']) !== false) {
                    if ($perfSuc == $Even['id_sucursal'] || $perfSuc == 'Todas') {
                        if ($Even['activa'] == 0) {
                            $strTabla.= "&nbsp;&nbsp;<img src='images/web_no.png' alt='Publicar la propiedad' title='Publicar la propiedad' border='0' style='cursor: pointer;' onclick=\"javascript: publicar(" . $Even['id_prop'] . ");\">";
                        } else {
                            $strTabla.= "&nbsp;&nbsp;<img src='images/web.png' alt='Publicar la propiedad' title='Publicar la propiedad' border='0' style='cursor: pointer;' onclick=\"javascript: publicar(" . $Even['id_prop'] . ");\">";
                        }
                        $strTabla.= "&nbsp;<input type='radio' name='id_prop' id='id_prop' value='" . $Even['id_prop'] . "' onclick='javascript:marcar(" . $Even['id_prop'] . ");'>";
                    }
                }
...
//                }
                ...
                $strTabla.= "  <div id='propCod' $onclick>";
//                $strTabla.= $Even['id_sucursal'] . str_repeat("0", 5 - strlen(strval($Even['id_prop']))) . $Even['id_prop'];
                $strTabla.= $Even['id_prop'];
                $strTabla.= "</div>\n";

                $strTabla.= "  <div id='propZona' $onclick>";
                $strTabla.= $txtZona;
                $strTabla.= "</div>\n";

                $strTabla.= "  <div id='propDir' $onclick>";
                $strTabla.= $Even['calle'] . "&nbsp;" . $Even['nro'] . "&nbsp;" . $Even['piso'] . "&nbsp;" . $Even['dpto'];
                $strTabla.= "</div>\n";

                $strTabla.= "  <div id='propTipo' $onclick>";
//                $strTabla.= (isset($tipoProp)) ? $tipoProp : '';
//                $strTabla.= $tipoProp;
                if (isset($arrTPop[$Even['id_tipo_prop']])) {
                    $strTabla.= $arrTPop[$Even['id_tipo_prop']];
                } else {
                    $strTabla.='';
                }
                $strTabla.= "</div>\n";

                $strTabla.= "  <div id='propSup' $onclick>";
                $strTabla.= number_format($Even['suptot'], 0, ',', '.');
                $strTabla.= "</div>\n";

                $strTabla.= "  <div id='propAmb' $onclick>";
                if ($Even['cantamb'] != "Sin definir") {
                    $strTabla.= number_format($Even['cantamb'], 0);
                }
                $strTabla.= "</div>\n";

                $strTabla.= "  <div id='propVal' $onclick>";
                if ($Even['monven'] != "Sin definir" && $Even['valven'] != 0) {
                    $strTabla.= $Even['monven'] . " " . number_format($Even['valven'], 0, ',', '.');
                } else {
                    if ($Even['monalq'] != "Sin definir") {
                        $strTabla.= $Even['monalq'] . " " . number_format($Even['valalq'], 0, ',', '.');
                    }
                }
                $strTabla.= "</div>\n";
                $strTabla.= "<div id=\"clearfix\"></div>\n";
            ...
            $strTabla.= "<ul>\n";
            $strTabla.= "<li><a class='boton_form_filtro' style='padding-top:5px;' href=\"javascript:ventana('lista_propiedadGoogle.php', 'Busqueda en GoogleMaps', 980, 900);\">Ver en GoogleMaps</a></li>\n";

            $strTabla.= "		<li align='right' colspan='7' style='padding-top:10px; padding-bottom:10px;'>";
            if ($pagina > 1) {
                $strTabla.= "    <a href='javascript:paginar(1);'>";
                $strTabla.= "       <img src='images/resultset_first.png' alt='Inicio' title='Inicio' border='0' style='vertical-align: middle;'></a>&nbsp;";
                if ($pagina > 2) {
                    $strTabla.= "    <a href='javascript:paginar(" . ($pagina - 1) . ");'>";
                    $strTabla.= "       <img src='images/resultset_previous.png' alt='Anterior' title='Anterior' border='0' style='vertical-align: middle;'></a>&nbsp;-&nbsp;";
                }
                for ($x = $pagina - 5; $x < $pagina; $x++) {
                    if ($x > 0) {
                        $strTabla.= "<a href='javascript:paginar(" . $x . ");'>$x</a>&nbsp;-&nbsp;";
                    }
                }
            }
            $strTabla.= "<span style='font-weight: bold; color:#FFF; background-color:#99BBEA;'>&nbsp;$pagina&nbsp;</span>-&nbsp;";
            if ($pagina < $paginas) {
                for ($x = $pagina + 1; $x < $pagina + 5; $x++) {
                    if ($x <= $paginas) {
                        $strTabla.= "<a href='javascript:paginar(" . $x . ");'>" . $x . "</a>&nbsp;-&nbsp;";
                ...
                if ($pagina < $paginas - 1) {
                    $strTabla.= "    <a href='javascript:paginar(" . ($pagina + 1) . ");'>";
                    $strTabla.= "       <img src='images/resultset_next.png' alt='Siguiente' title='Siguiente' border='0' style='vertical-align: middle;'></a>&nbsp;";
                }
                $strTabla.= "    <a href='javascript:paginar($paginas);'>";
                $strTabla.= "       <img src='images/resultset_last.png' alt='Ultima' title='Ultima' border='0' style='vertical-align: middle;'></a>&nbsp;";
            }
            $strTabla.= "</li>";
            $strTabla.= "</ul>";
            $strTabla.= "  </div>\n";
    ...
    public function armaMailSeleccion($arrayDatos, $idCli, $arrayEnvio) {
 ...
 */
        foreach ($arrayDatos as $propiedad) {
            ...
            if (in_array($id_prop, $arrayEnvio)) {
            // BUSCA FOTOS ----------
            $fotoBSN = new FotoBSN ();
...
//			}
            ...
            if (isset($arrayFoto [0]) && $arrayFoto [0] ['hfoto'] != "") {
                $mensaje .= "      <img src=\"http://abm.okeefe.com.ar/fotos_th/" . $arrayFoto [0] ['hfoto'] . "\" width=\"330\" border=\"0\" style=\"cursor: pointer;\" />\n";
            } else {
            ...
            if (isset($arrayFoto [1]) && $arrayFoto [1] ['hfoto'] != "") {
                $mensaje .= "      <img src=\"http://abm.okeefe.com.ar/fotos_th/" . $arrayFoto [1] ['hfoto'] . "\" width=\"157\" border=\"0\" style=\"cursor: pointer;\" />\n";
            }
            $mensaje .= "    </a></td>\n";
            $mensaje .= "    <td><a href=\"http://www.okeefe.com.ar/detalleProp.php?iddest=".$id_prop."\">\n";
            if (isset($arrayFoto [2]) && $arrayFoto [2] ['hfoto'] != "") {
                $mensaje .= "      <img src=\"http://abm.okeefe.com.ar/fotos_th/" . $arrayFoto [2] ['hfoto'] . "\" width=\"157\" border=\"0\" style=\"cursor: pointer;\" />\n";
            }
            $mensaje .= "   </a></td>\n";
            $mensaje .= "  </tr>\n";
            $mensaje .= "  <tr>\n";
            ...
            $mensaje .= "      <tr>\n";
            $mensaje .= "        <td><div><span style=\"font-weight:bold;\">Direcci&oacute;n:</span> <span class=\"caracResul\">" . $propiedad['calle'] . " " . $propiedad['nro'] . " " . $ubi . "</span><br />\n";
...
//        $cliMail=$cliBSN->getObjeto()->getEmail();
        ...
        if (mail($para, $Subject, $mensaje, $headers)) {
            if ($idCrm != 0) {
                ...
                foreach ($arrayDatos as $propiedad) {
                    $retorno = $crmBSN->actualizaEstadoAdjunto($propiedad['id_prop'], 2);
...
//            echo "Error al enviar";
            $retEnvio = 0;  // Poner valor en 1 para cuando el envio funciona
        }
        return $retEnvio;
        //-------------------------------------------
    }
            
    public function armaMailSeleccion_ORIG() {
        include_once('clases/class.loginwebuserBSN.php');
        include_once ("clases/class.fotoBSN.php");
        include_once("clases/class.clienteBSN.php");
        include_once("inc/class.mail.php");
...
//			}
            ...
            $mensaje .= "    <td><table width=\"100%\" border=\"0\" cellspacing=\"5\" cellpadding=\"0\">\n";
            $mensaje .= "  <tr>\n";
            $mensaje .= "    <td width=\"680\" style=\"font-weight: bold; font-size:12pt;color:#008046;\">" . substr($arrayDatos [0] ['contenido'], 0, 100) . "</td>\n";
            $mensaje .= "    <td width=\"80\" align=\"center\" style=\"font-weight: bold; padding:5px; border: thin solid #008046;border-radius: 5px;-ms-border-radius: 5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;-khtml-border-radius: 5px;\">ID " . str_repeat("0", 5 - strlen(strval($id_prop))) . $id_prop . "</td>\n";
            $mensaje .= "    <td width=\"80\" align=\"center\" style=\"font-weight: bold; padding:5px; color:#FFF; background-color: #008046; border-radius: 5px;-ms-border-radius: 5px;-moz-border-radius: 5px;-webkit-border-radius: 5px;-khtml-border-radius: 5px;\">" . $propiedad ['operacion'] . "</td>\n";
            $mensaje .= "  </tr>\n";
            $mensaje .= "</table>\n";
