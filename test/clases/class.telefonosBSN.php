...
require_once 'clases/class.tipoTelefonoBSN.php';
...
//    protected $arrayTipos = array('Particular', 'Cel Particular', 'Laboral', 'Cel Laboral', 'Familiar', 'Mensages', 'Fax');
    ...
    protected function seteaArrayTipos(){
        $tipoTel = TipotelefonoBSN::getInstance();
        $this->arrayTipos=$tipoTel->getArrayParametros();
    }
    ...
    public function comboTipotel($valor = '', $campo = "tipotel", $class = "cd_celda_input") {
        $tipoTel=  TipotelefonoBSN::getInstance();
        $tipoTel->comboParametros($valor , 0, $campo, $class,'');
        /*
    ...
    /**
     * Retorna una coleccion con los valores de telefono definidos para el contacto del ID en un campo formateado
     * @param int $_id -> id del contacto
     * @return string[] -> array con los datos del telefono de contacto
     */
    public function coleccionByClienteString($_id = 0) {
        $arrayRet = array();
        $arrayDatos=array();
        if ($_id != 0) {
            $arrayDatos = $this->coleccionByContacto('C', $_id, 0);
        }
        foreach ($arrayDatos as $tel){
            $arrayRet[]=  $this->registro2string($tel);
        }
        return $arrayRet;
    }
    /**
 ...
 */
            $strRet.=$this->registro2string($telefono);
        }
        return $strRet;
    }

    protected function registro2string($telefono){
        $strRet='';
        if ($telefono['principal'] != '' && $telefono['principal'] != '0') {
            $strRet.="<strong>";
        }
    ...
    public function armaVistaTelefonos($id_cli=0){
        $strRet = '';
        $arrayRet = array();
        if ($id_cli != 0) {
            $arrayRet = $this->coleccionByCliente($id_cli);
            if (sizeof($arrayRet) != 0) {
                $strRet='<table>';
                $template = new Template("listaTelefonos.html");
                $patron=$template->getSalida();
                foreach ($arrayRet as $telefono){
                    $template->setTemplateSalida($patron);
                    $template->parsearArray($telefono);
                    $template->parsearDato('TIPO_TEL', $this->arrayTipos[$telefono['tipotel']]);
