...
class UbicacionpropiedadBSN extends BSN {
    ...
    public function getJsonComboZonaPrincipal() {
        $zona = $this->cargaColeccionPrincipal();
        $arrayRet = array();
        for ($pos = 0; $pos < sizeof($zona); $pos++) {
            $arrayRet[] = array($zona[$pos]['id_ubica'], $zona[$pos]['nombre_ubicacion']);
        }
        return $this->getJsonColeccion($arrayRet);
    }
    ...
    public function comboUbicacionpropiedadPrincipal2Txt($valor = 0, $opcion = 0, $campo = "id_ubicaPrincipal", $campoUbica = 'id_ubica', $divTxtUbica = 'txtUbica', $campoEmp = '', $class = "campos_btn", $divUbicacion = '') {
        $txt = '';
        $arrayPricipal = $this->cargaColeccionPrincipal();
        $txt.=( "<select name='" . $campo . "' id='" . $campo . "' class='" . $class . "' ");
        if ($opcion != 3) {
            $txt.=( " onblur=\"javascript: document.getElementById('$campoUbica').value=document.getElementById('$campo').value;document.getElementById('$divTxtUbica').innerHTML='';\" onchange=\"javascript: document.getElementById('$campoUbica').value=document.getElementById('$campo').value;document.getElementById('$divTxtUbica').innerHTML='';\"");
        } else {
            $txt.=( " onblur=\"javascript: document.getElementById('$campoUbica').value=document.getElementById('$campo').value;document.getElementById('$divTxtUbica').innerHTML='';\" onchange=\"javascript: document.getElementById('$campoUbica').value=document.getElementById('$campo').value;document.getElementById('$divTxtUbica').innerHTML='';\"");
        }
        ...
        switch ($opcion) {
            ...
            case 2:
                ...
                $txt.=( ">Seleccione una opcion</option>\n");
                break;
        ...
        for ($pos = 0; $pos < sizeof($arrayPricipal); $pos++) {
            $txt.=( "<option value='" . $arrayPricipal[$pos]['id_ubica'] . "'");
            if (($arrayPricipal[$pos]['id_ubica'] == $valor) || ($arrayPricipal[$pos]['id_ubica'] == 3979 && $valor == 0 && $all == 0)) {
                $txt.=( " SELECTED ");
            }
            $txt.=( ">" . $arrayPricipal[$pos]['nombre_ubicacion'] . "</option>\n");
        }
        $txt.=( "</select>\n");
        return $txt;
    }
