...
class IngresoclienteVW extends VW {
    ...
    public function __construct($_id = 0, $tipo = 'I') {
        ...
        if ($_id != 0 && $tipo == 'C') { // Ingreso el Id de un cliente
            IngresoclienteVW::cargaByCliente($_id);
        } else {
            if (is_numeric($_id)) {
                if ($_id != 0) {
...
// Cargo los datos del ingreso de un cliente segun su ID    
    protected function cargaByCliente($id_cli) {
        $ingBSN = new IngresoclienteBSN();
        $datos = $ingBSN->cargaByCliente($id_cli);
        $this->seteaVW($datos);
    }

    public function cargaDatosVW($operacion = 'n') {
        if (!isset($this->arrayForm['id_ingreso']) || (isset($this->arrayForm['id_ingreso']) && ($this->arrayForm['id_ingreso'] == '' || $this->arrayForm['id_ingreso'] == 0))) {
            ...
            //print "<div  class=\"nombreCampo\">Fecha de ingreso</div><div class=\"datoCampo\"><input type='text' readonly='readonly' name='fec_cont' id='fec_cont' value='" . $this->arrayForm['fec_cont'] . "' /></div>\n";
            $fechaContacto = "el ".$this->arrayForm['fec_cont'];
            
        }
        print "        <div class=\"col3\">\n";
        print "          <div class=\"nombreCampo\">Forma de primer contacto &bull;".$fechaContacto."</div>\n";
        print "          <div>\n";
        $fContBSN = FormacontactoBSN::getInstance();
        $fContBSN->comboParametros($this->arrayForm ['id_fcontacto'], 0, 'id_fcontacto', 'datoCampo');
        print "          </div>\n";
        print "        </div>\n";
        ...
        if ($operacion == 'n') {
            print "<input type='hidden' name='id_asunto' id='id_asunto' value='" . date('YmdHis') . "'>\n";

            print "      <div>\n";
            $fIngBSN = FormaingresoBSN::getInstance();
            $fIngBSN->selectorFormaIngreso($this->arrayForm ['id_promo'], 'id_fingreso');
            print "    </div>\n";
        }
    ...
    public function cargaDatos2VW() {
        ...
        print "<br>$titulo<br>";
        print "<input type='text' size='50' name='buscaPromoCli' id='buscaPromoCli' >";
