...
class FamiliaresPGDAO extends PGDAO {
	...
	protected $INSERT="INSERT INTO #dbName#.familiares (fecha_nac,id_cli,nombre,apellido,id_parent,nota,tipocont) values (STR_TO_DATE('#fecha_nac#', '%d/%m/%Y'),#id_cli#,'#nombre#','#apellido#','#id_parent#','#nota#','#tipocont#')";
	protected $DELETE="DELETE FROM #dbName#.familiares WHERE id_fam=#id_fam#";
	protected $UPDATE="UPDATE #dbName#.familiares SET fecha_nac=STR_TO_DATE('#fecha_nac#', '%d/%m/%Y'), id_cli='#id_cli#',nombre='#nombre#',apellido='#apellido#',id_parent='#id_parent#',nota='#nota#',tipocont='#tipocont#' WHERE id_fam=#id_fam# ";
