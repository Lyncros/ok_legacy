...
class RelacionBSN extends BSN {
     ...
     */
    public function borraDB($param=0) {
        if($param instanceof Relacion){
        $relDB = new RelacionPGDAO();
            $_id_pc = $param->getId_pc();
            $_id_sc = $param->getId_sc();
            $_id_rel = $param->getId_relacion();
...
//        echo $tipo." - ".$id."<br>";
    ...
    public function cargaRelacionesCC($id){
        $tipos = new TiporelacionBSN();
        $listaTipos1 = $tipos->armaListaRelacionesByTipo('CC');

        $relDB = new RelacionPGDAO();

        $result1 = $relDB->relacionByParte($id, 1, $listaTipos1);
        $arrayRet1 = $this->leeDBArray($result1);
        $retorno1 = $this->cargaDatosRelacionesReducida($id, 'C', 1, $arrayRet1);
        
        $result2 = $relDB->relacionByParte($id, 2, $listaTipos1);
        $arrayRet2 = $this->leeDBArray($result2);
        $retorno2 = $this->cargaDatosRelacionesReducida($id, 'C', 2, $arrayRet2);
    ...
    public function cargaRelacionesPropiedadByCliente($id_cli){
        $tipos = new TiporelacionBSN();
        $listaTipos1 = $tipos->armaListaRelacionesByTipo('CP');

        $relDB = new RelacionPGDAO();

        $result1 = $relDB->relacionByParte($id_cli, 1, $listaTipos1);
        $arrayRet1 = $this->leeDBArray($result1);
        $retorno = $this->cargaDatosRelacionesReducida($id_cli, 'C', 1, $arrayRet1);
    ...
    /**
     * Retorna los datos de la relacion del Cliente respecto a los demas componentes
     * @param type $id_cli
     * @param type $contraparte 'u' usuario  'c' cliente 'p' propiedad '' todas
     * @return string
     */
    public function armaVistaRelacionCliente($id_cli=0,$contraparte=''){
        $strRet = '';
        $arrayRet = array();
        if ($id_cli != 0) {
            switch ($contraparte){
                case 'c';
                    //Array ( [0] => Array ( [id_comp] => 410 [orden] => 1 [desc_comp] => MARTIN ABASCAL [id_rel] => 4 [desc_rel] => Familiar - Padre ) )
                    $arrayRet = $this->cargaRelacionesCC($id_cli);
                    if (sizeof($arrayRet) != 0) {
                        $template = new Template("listaRelaciones.html");
                        $patron=$template->getSalida();
                        $telBSN=new TelefonosBSN();
                        $medBSN=new MedioselectronicosBSN();
                        $cliBSN=new ClienteBSN();
                        foreach ($arrayRet as $dato){
                            $template->setTemplateSalida($patron);
                            $template->parsearArray($dato);
                            
                            $cliBSN->cargaById($dato['id_comp']);
                            $template->parsearDato('EMPRESA', $cliBSN->getObjeto()->getEmpresa());
                            
                            $template->parsearDato('TELEFONO', $telBSN->listaTelefonosByCliente($dato['id_comp']));
                            $template->parsearDato('EMAIL', $medBSN->listaMedioselectronicosByCliente($dato['id_comp']));
                    ...
                    break;
                case 'u':
                    //Array ( [0] => Array ( [0] => 100 [id_pc] => 100 [1] => 20120529112355 [id_sc] => 20120529112355 [2] => 11 
                    //[id_relacion] => 11 [desc_pc] => Camila Varela Gomez [desc_sc] => KARINA BARRETO [desc_rel] => Ejecutivo cuenta - Primer ) )
                    $arrayRet = $this->coleccionRelacionesUC(0,$id_cli);
                    if (sizeof($arrayRet) != 0) {
                        $strRet='<table>';
                        $template = new Template("listaEjecutivos.html");
                        $patron=$template->getSalida();
                        foreach ($arrayRet as $dato){
                            $template->setTemplateSalida($patron);
                            $template->parsearArray($dato);
                    ...
                    break;
                case 'p':
                    //( [id_comp] => 130 [orden] => 2 [desc_comp] => [130] ANDRES BARANDA 
                    // (Bs.As. G.B.A. Zona Sur, Quilmes, Quilmes Oeste) [id_rel] => 1 [desc_rel] => Propietario )
                    $arrayRet = $this->cargaRelacionesPropiedadByCliente($id_cli);
                    if (sizeof($arrayRet) != 0) {
                        $template = new Template("listaPropiedades.html");
                        $propBSN=new PropiedadBSN();
                        $tipoPropBSN = new Tipo_propBSN();
                        $arrayTipoProp=$tipoPropBSN->armaArrayTipoProp();
                        $patron=$template->getSalida();
                        foreach ($arrayRet as $dato){
                            $template->setTemplateSalida($patron);
                            $template->parsearArray($dato);

                            $propBSN->cargaById($dato['id_comp']);
                            $template->parsearDato('TIPO_PROP',$arrayTipoProp[$propBSN->getObjeto()->getId_tipo_prop()] );
//                            $template->parsearDato('{TIPO_PROP}',$dato['id_comp'] );
                    ...
                    break;
                default:
                    $arrayRet = $this->cargaRelacionesCliente($id_cli);
                    break;
            }
        }
        return $strRet;
    ...
    public function armaCargaRelacionClientePropiedad($id_cli){
        $template = new Template("cargaRelacionClientePropiedad.html");
        $tipoRel=new TiporelacionBSN();
        $strCombo=$tipoRel->comboRelacionStr(0, 'CP');
        $template->parsearDato('ID_CLI', $id_cli);
        $template->parsearDato('AUX_PROP', '');
        $template->parsearDato('COMBORELACION', $strCombo);
        return $template->getSalida();
    }
    
    public function armaCargaRelacionClienteCliente($id_cli){
        $template = new Template("cargaRelacionClienteCliente.html");
        $tipoRel=new TiporelacionBSN();
        $strCombo=$tipoRel->comboRelacionStr(0, 'CC');
        $template->parsearDato('ID_CLI', $id_cli);
        $template->parsearDato('AUX_CLI', '');
        $template->parsearDato('COMBORELACION', $strCombo);
        return $template->getSalida();
    }
...
//        print "<div id='cargaDataRelacion' name='cargaDataRelacion' style='display:none;'>";
        print "<div id='cargaDataRelacion' name='cargaDataRelacion' style='display:block;'>";
