...
class UbicacionesActivas {
    ...
    public function armaColeccionNombresActivas() {
        $arrayRet = array();
        $arrayKeys = array_keys($this->arrayActivas);
        for ($x = 0; $x < sizeof($this->arrayActivas); $x++) {
            $activas = $this->arrayActivas[$arrayKeys[$x]];
            ...
            if ($activas[$operacion][$tipo_prop] != 0) {
                ...
                $arrayRet[] = array('id_ubica' => $arrayKeys[$x], 'nombre_ubicacion' => $this->armaNombreCompleto($arrayKeys[$x]));
    }
        }
        return $arrayRet;
    }
    ...
    /**
     * Retorna un array JSON con los datos para el autocompletar de Zona
     */
    public function armoAutocompletar($filtro) {
        $result = array();
        $items = $this->armaColeccionNombresActivas();
        foreach ($items as $item) {
            if (strpos(strtolower($item['nombre_ubicacion']), strtolower($filtro)) !== false) {
                array_push($result, array("id" => $item['id_ubica'], "label" => $item['nombre_ubicacion'], "value" => strip_tags($item['nombre_ubicacion'])));
            }
        }
        return $result;
    }
    ...
    protected function armaNombreCompleto($id) {
        if ($this->arrayActivas[$id]['id_padre'] == 0) {
            return $this->arrayActivas[$id]['nombre_ubicacion'];
        } else {
            $nombre = $this->arrayActivas[$id]['nombre_ubicacion'];
            $nombre = $this->armaNombreCompleto($this->arrayActivas[$id]['id_padre']) . ", " . $nombre;
}
        return $nombre;
    }
