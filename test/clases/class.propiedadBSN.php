...
include_once("clases/class.propiedadCache.php");
...
//		$array = $this->leeDBArray($datoDB->coleccionByFiltroBuscadorMapa($codigo,$calle,$id_ubica,$tipo_prop,$operacion,$id_emp,$in,$pagina,$registros,$campo,$orden,$vistaestado,$vistazona,$publicadas));
        $array = $this->leeDBArray($datoDB->coleccionByFiltroBuscadorMapa($codigo, $calle, $idsZonas, $tipo_prop, $operacion, $id_emp, $in, $pagina, $registros, $campo, $orden, $vistaestado, $vistazona, $publicadas));
//        print_r($array);
        $arrayform = array();
        foreach ($array as $registro) {
            $arrayform[] = $this->mapa->tablaTOform($registro);
        }
//        echo "********************************************<br>";print_r($arrayform);
        return $arrayform;
    }
    ...
    public function cargaAutocompletarAlquileres($buscar){
        $res=$this->cargaColeccionAlquileres($buscar);
        $arrayRet=array();
        foreach ($res as $prop){
            $etiq=trim($prop['calle']).' '.trim($prop['nro']).' '.trim($prop['piso']).' '.trim($prop['dpto']);
            $arrayRet[]=array('id'=>$prop['id_prop'],'label'=>$etiq,'value'=>$etiq);
        }
        return $arrayRet;
    }
    
    public function cargaColeccionAlquileres($buscar=''){
        $datoDB = new PropiedadPGDAO();
        $array = $this->leeDBArray($datoDB->coleccionAlquileres($buscar));
//        $arrayform = array();
//        foreach ($array as $registro) {
//            $arrayform[] = $this->mapa->tablaTOform($registro);
//        }
//        return $arrayform;
    ...
    public function cantidadRegistrosFiltroBuscador($codigo, $calle, $id_ubica, $tipo_prop, $operacion, $id_emp, $in, $publicadas=-1, $aux_vistaestado='') {
        $ubiBSN = new UbicacionpropiedadBSN();
        $retorno = 0;
...
//		$reg = $propPGDAO->cantRegistrosFiltroBuscador($codigo,$calle,$id_ubica,$tipo_prop,$operacion,$id_emp,$in);
     ...
     */
    public function cargaColeccionZonasActivas() {
        $datoDB = new PropiedadPGDAO();
        $operacion = "'Venta','Alquiler','Alquiler Temporario','Alquiler o Venta'";
        $activas = -1;
        $array = $this->leeDBArray($datoDB->coleccionZonasActivas($operacion, $activas));
        return $array;
    }
    ...
    public function buscaDescripcionPropiedadCorto() {
        $desc = '';
        $ubicaBSN = new UbicacionpropiedadBSN($this->propiedad->getId_ubica());
        $desc = "[" . $this->propiedad->getId_prop() . "] " . $this->propiedad->getCalle() . " " . $this->propiedad->getNro() . " (" . $ubicaBSN->armaNombreZonaAbr($this->propiedad->getId_ubica()) . ")";

        return $desc;
    }
    
    public function buscaDetallePropiedad($conOp=1) {
        $retorno = '';
    ...
    public function actualizaCache($modo){
        $cache= PropiedadCache::getInstance();
        $cache->actualizaCache($this->getId(),$modo);
    }
    
    public function marcaAlquilado($alquilado=1) {
        $objDB = new PropiedadPGDAO($this->getArrayTabla());
        $arrayRet = $this->leeDBArray($objDB->marcaAlquilado($alquilado));
        return $arrayRet;
}
