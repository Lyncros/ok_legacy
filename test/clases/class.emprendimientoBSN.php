...
class EmprendimientoBSN extends BSN {
    ...
    public function cantidadRegistrosFiltro($id_ubica,$tipo_emp,$estado='',$activa=-1) {
    	$cant=0;
        $propPGDAO=new EmprendimientoPGDAO();
        $reg = $propPGDAO->cantidadRegistrosFiltro($id_ubica,$tipo_emp,$estado,$activa=-1);
     ...
     */
    public function comboEmprendimiento($valor=0,$opcion=0,$id_ubica=0,$campo="id_emp",$class="campos_btn") {
        $emp=$this->cargaColeccionFiltro($id_ubica,0,-1,0,0);
//        echo"ENTRA";print_r($emp);
    ...
    public function checkboxEmprendimiento($valor,$id_ubica=0){
        if($id_ubica!=0){
                $ubiBSN = new UbicacionpropiedadBSN();
                $id_ubica=$ubiBSN->armaListaSeleciones($id_ubica, '');
        }
        $emprendimiento=$this->cargaColeccionFiltro($id_ubica, 0,-1,0,0);
