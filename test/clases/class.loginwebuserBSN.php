...
//		$this->loginwebuser->setNueva_clave($_claveEnc);
    ...
    public function buscaDescripcionUsuario() {
        $desc = '';
        $desc = trim($this->loginwebuser->getNombre()) . " " . trim($this->loginwebuser->getApellido());
        return $desc;
    }
...
//        $medio=$medElec->listaMedioselectronicosByCliente($id);
  ...
  //      }
    ...
    public function getJsonUsuarios(){
        $usrBSN=new LoginwebuserBSN();
        $arrayUsr=$usrBSN->cargaColeccionForm();
        $arrayJs=array();
        foreach ($arrayUsr as $usr){
            $arrayJs[]=array($usr['id_user'],$usr['nombre']." ".$usr['apellido']);
        }
        return json_encode($arrayJs);
    }
