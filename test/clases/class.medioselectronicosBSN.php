...
class MedioselectronicosBSN extends BSN {
    ...
    public function coleccionByClienteString($_id = 0) {
        ...
        if ($_id != 0) {
            $arrayDato = $this->coleccionByContacto('C', $_id, 0);
        }
        foreach ($arrayDato as $medio){
            $arrayRet[]=  $this->registro2string($medio,$arrayMedio);
        }
        return $arrayRet;
    }
 ...
 */
            $strRet.=$this->registro2string($medio,$arrayMedio);
        }
        return $strRet;
    }

    protected function registro2string($medio,$arrayMedio){
        $strRet='';
        if ($medio['principal'] != '' && $medio['principal'] != '0') {
            $strRet.="<strong>";
        }
    ...
    public function armaVistaMedios($id_cli=0){
        $strRet = '';
        $arrayRet = array();
        if ($id_cli != 0) {
            $tipoMed = TipoMedioElectronicoBSN::getInstance();
            ...
            if (sizeof($arrayRet) != 0) {
                $strRet='<table>';
                $template = new Template("listaMedios.html");
                $patron=$template->getSalida();
                foreach ($arrayRet as $medios){
                    $template->setTemplateSalida($patron);
                    $template->parsearArray($medios);
                    $template->parsearDato('TIPO_MED', $arrayMedio[$medios['id_tipomed']]);
