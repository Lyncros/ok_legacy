...
class PropiedadcontratoBSN extends BSN {
    ...
    public function armaListaPropiedadcontratoByTipoEstado($tipo='',$estado='a'){
        $lista=array();
        $propBSN=new PropiedadBSN();
        if($estado=='a'){
            $lista=  $this->cargaPropiedadcontratoActivos($tipo);
        }else{
            $lista=  $this->cargaPropiedadcontratoVencidos($tipo);
        }
        $largo=  sizeof($lista);
        for($x=0;$x < $largo ; $x++){
//            $contra=$lista[$x];
            $propBSN->cargaById($lista[$x]['id_prop']);
            $lista[$x]['propiedad']=$propBSN->buscaDescripcionPropiedadCorto();
        }
        return $lista;
    }
