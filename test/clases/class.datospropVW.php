...
class DatospropVW {
	...
	public function cargaDatosDatosprop($id_prop,$pagorig) {
		...
		if($id_prop != 0 && $id_prop != ""){
			$tit = "Modificación de Caracteristicas";
		}else{
			$tit = "Carga de Caracteristicas";
		}

		print "<table width='100%' align='center' bgcolor='#f1f1f1'>\n";

		print "<tr><td class='cd_celda_titulo'>$tit</td></tr>\n";
		...
		foreach ($arrayCarac as $elemCarac) {
			if($tipoCarac != $elemCarac[1]) {
				...
				print "<table width='100%' align='center' cellpadding='5'>\n";
				print "<tr><td class='celda_titu_carac'>".$elemCarac[1]."</td></tr>\n";
				$tipoCarac=$elemCarac[1];
				$cols=$elemCarac[2];
				$col=0;
				$ancho=intval(100/$cols);
				print "<tr><td style='padding-top:5px;'><table cellspacing='10' width='100%'>\n";
			}
			if($col==$cols) {
				print "</tr>\n";
				$col=0;
			...
			switch ($carac->getTipo()) {
				case 'CheckBox':
					...
					break;
				case 'Lista':
					$this->armaLista($carac->getId_carac(),$carac->getTitulo(),$carac->getLista(),$arrayDatos[0]['contenido'],$carac->getComentario(),$arrayDatos[0]['comentario'],$ancho);
					break;
				case 'Numerico':
	...
	/**
	 * Arma un div con los campos de las caracteristicas especificas para el tipo de propiedad que se ha definido
	 * @param unknown_type $id_prop
	 * @param unknown_type $tipo_prop
	 */
	public function cargaDatosDatospropDiv($id_prop,$tipo_prop,$div='datosprop') {

		$caracBSN = new CaracteristicaBSN();
		$carac = new Caracteristica();
		$auxiliar = new AuxiliaresPGDAO();
		$datosProp2 = new Datosprop();
		$arrayCarac = $auxiliar->coleccionTipopropCarac($tipo_prop);
		$arrayTipocarac= $auxiliar->coleccionTipopropTipocarac($tipo_prop);

		print "<script type='text/javascript' language='javascript'>\n";
		print "function direccionaTipocarac(campo){\n";
		print "		location.href='#'+campo.value;\n";
		print "}\n";
		print "</script>\n";
		...
		print "<tr><td class='cd_celda_titulo'><a name='titulo'>Caracteristicas de la Propiedad</a></td></tr>\n";
		print "<tr><td class='celda_titu_carac'> Ir a las caracteristicas: ";
		print "<select class='campos_btn' onchange='javascript: direccionaTipocarac(this);'>";
		foreach ($arrayTipocarac as $tipo_carac) {
			print "<option value='".$tipo_carac[1]."'>".$tipo_carac[1]."</option>";
		}
		print "</select>";
		print "</td></tr>\n";
		...
		foreach ($arrayCarac as $elemCarac) {
			if($tipoCarac != $elemCarac[1]) {
				if($tipoCarac!='') {
					print "</tr></table>";
					print "<a href='#titulo'><img src='images/up.png' alt='subir'></a>Ir al tope ...";
					print "</td>\n</tr>\n</table></td></tr>\n";
				}
				print "<tr><td align='center' style='padding-bottom:20px;'>";
				print "<table width='100%' align='center' cellpadding='5'>\n";
				print "<tr><td class='celda_titu_carac'>";
				print "<a name='".$elemCarac[1]."'>".$elemCarac[1]."</a></td></tr>\n";
				$tipoCarac=$elemCarac[1];
				$cols=$elemCarac[2];
				$col=0;
				$ancho=intval(100/$cols);
			...
			$datosPropBSN->seteaBSN($datosProp2);
			$arrayDatos=$datosPropBSN->cargaColeccionForm();
                        if(array_key_exists(0,$arrayDatos)){
			...
			switch ($carac->getTipo()) {
				case 'CheckBox':
					$this->armaCheckbox($carac->getId_carac(),$carac->getTitulo(),$contenido,$carac->getComentario(),$comentario,$ancho);
//					$this->armaCheckbox($carac->getId_carac(),$carac->getTitulo(),$arrayDatos[0]['contenido'],$carac->getComentario(),$arrayDatos[0]['comentario'],$ancho);
					break;
				case 'Lista':
					$this->armaLista($carac->getId_carac(),$carac->getTitulo(),$carac->getLista(),$contenido,$carac->getComentario(),$comentario,$ancho);
//					$this->armaLista($carac->getId_carac(),$carac->getTitulo(),$carac->getLista(),$arrayDatos[0]['contenido'],$carac->getComentario(),$arrayDatos[0]['comentario'],$ancho);
					break;
				case 'Numerico':
					$this->armaNumerico($carac->getId_carac(),$carac->getTitulo(),$carac->getMaximo(),$carac->getComentario(),$contenido,$comentario,$ancho);
//					$this->armaNumerico($carac->getId_carac(),$carac->getTitulo(),$carac->getMaximo(),$carac->getComentario(),$arrayDatos[0]['contenido'],$arrayDatos[0]['comentario'],$ancho);
					break;
				case 'Texto':
					$this->armaTexto($carac->getId_carac(),$carac->getTitulo(),$contenido,$ancho);
//					$this->armaTexto($carac->getId_carac(),$carac->getTitulo(),$arrayDatos[0]['contenido'],$ancho);
					break;
				case 'Texto Largo':
					$this->armaTextoLargo($carac->getId_carac(),$carac->getTitulo(),$contenido,$ancho);
//					$this->armaTextoLargo($carac->getId_carac(),$carac->getTitulo(),$arrayDatos[0]['contenido'],$ancho);
					break;
				case 'Web':
					$this->armaWeb($carac->getId_carac(),$carac->getTitulo(),$contenido,$ancho);
//					$this->armaWeb($carac->getId_carac(),$carac->getTitulo(),$arrayDatos[0]['contenido'],$ancho);
					break;
				case 'Usuarios':
                    $this->armaListaUsuarios($carac->getId_carac(),$carac->getTitulo(),$contenido, $ancho);                                        
					break;
			}
		}
		print "</tr></table></td>\n</tr></table></td>\n</tr>\n";
		if(sizeof($arrayTipocarac)!=0){
			print "<tr><td align='right'><input class='boton_form' type='submit' value='Enviar'><br /><br /><span class='obligatorio'>Los campos marcados con &bull; son obligatorios</span></td></tr>\n";
		}
		print "</div>\n";
	}
	...
	public function vistaDatosPropBuscador($id_prop) {
		...
		foreach ($arrayCarac as $elemCarac) {
			...
			$arrayDatos=$datosPropBSN->cargaColeccionForm();
                        if(isset($arrayDatos[0])){
                            if($arrayDatos[0]['contenido'] != "" && $arrayDatos[0]['contenido']!= "Sin definir" && $arrayDatos[0]['contenido']!= "off") {
                                if($tipoCarac != $elemCarac[1]) {
                                    if($tipoCarac!='') {
                                            print "</tr></table></td>\n</tr>\n</table></td></tr>\n";
                                    }
                                    print "<tr><td align='center' style='padding-bottom:20px;'>";
                                    print "<table width='100%' align='center' bgcolor='#FFFFFF'>\n";
                                    print "<tr><td class='datos_resumen'><img src=\"images/flechita.gif\" width=\"16\" height=\"16\" style=\"vertical-align:middle;\" /> ".$elemCarac[1]."</td></tr>\n";
                                    $tipoCarac=$elemCarac[1];
                                    print "<tr><td><table width=\"100%\" border=\"0\" cellspacing=\"2\" cellpadding=\"0\">\n";
                                }

                                    print "<tr>\n";
                                    print "<td><table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\">\n";
                                    print "      <tr class=\"tabla_ficha\">\n";
                                    print "           <td class=\"detalle_ficha\" width=\"55%\">" . $carac->getTitulo() . "</td>\n";
                                    print "           <td class=\"data_ficha\">";
                                    switch ($carac->getTipo()) {
                                        case 'CheckBox':
                                            echo  ($arrayDatos[0]['contenido'] == "on") ? "<img src='images/tilde.png' width='14' height='15'>" : "<img src='images/tilde_no.png' width='14' height='15'>";
                                            break;
                                        case 'Lista':
                                            echo $arrayDatos[0]['contenido'];
                                            break;
                                        case 'Numerico':
                                            echo number_format($arrayDatos[0]['contenido'],2, '.', ',');
                                            break;
                                        case 'Texto':
                                            echo $arrayDatos[0]['contenido'];
                                            break;
	...
	protected function armaLista($id_carac,$titulo,$lista,$contenido,$comentario,$contcoment,$ancho) {
		$array=split(';',$lista);
		$campo='l_'.$id_carac;
		//        $ancho-=20;
	...
	protected function armaNumerico($id_carac,$titulo,$maximo,$comentario,$contenido,$contcoment,$ancho) {
		print "<td class='cd_celda_texto'>$titulo<br />";
		//        $ancho-=20;
	...
	protected function armaCampo($campo,$contenido) {
		print "<input class='campos' type='text' name='$campo' id='$campo' value='".	$contenido ."' maxlength='200'>";
	}
	...
	protected function armaCheckbox($id_carac,$titulo,$contenido,$comentario,$contcoment,$ancho) {
		...
		if($comentario=='Si') {
			...
			$campo='com_'.$id_carac;
			print $titulo . "<br />";
			print "<input class='campos' type='text' name='".$campo."' id='$campo' value='".$contcoment ."' maxlength='200' style='width:90%;'>";
		}else{
			print "<input type='checkbox' id='".$campo."' name='".$campo."'";
			if ($contenido=='on') {
				print " checked ";
	...
	protected function armaTextoLargo($id_carac,$titulo,$contenido,$ancho) {
		...
		$campo='b_'.$id_carac;
		//		$this->armaCampo($campo,$contenido);
		print "<textarea rows='6' class='campos_area' name='$campo' id='$campo'>".	$contenido ."</textarea>";
		print "</td>\n";
	}
	...
	protected function armaWeb($id_carac,$titulo,$contenido,$ancho) {
		...
		$ckfinder->SetupCKEditorObject($rFCKeditor);	
		
		print "<td class='cd_celda_texto'>$titulo<br />";
		$ancho-=20;
		print "<td width='$ancho%'>";
		//$rFCKeditor->Create() ;
		$rFCKeditor->editor($campo);
		print "</td>\n";
	}

    public function armaFiltroDatosprop2Txt($array, $tipo_prop) {
        $txtFiltro = '';
        $caracBSN = new CaracteristicaBSN();
        ...
        foreach ($arrayCarac as $elemCarac) {
            ...
            switch ($elemCarac['tipo']) {
                case 'CheckBox':
                    $txtFiltro.=$this->armaCheckboxFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, '', '', 80);
                    break;
                case 'Lista':
                    $txtFiltro.=$this->armaListaFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $elemCarac['lista'], $valor, 80);
                    break;
                case 'Numerico':
                    $compara = $caracBSN->cargaComparacionCarac($elemCarac['id_carac']);
                    $txtFiltro.=$this->armaNumericoFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $elemCarac['maximo'], '', $valor, '', 80, $compara);
                    break;
                case 'Texto':
                    $txtFiltro.=$this->armaTextoFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, 100);
                    break;
                case 'Texto Largo':
                    $txtFiltro.=$this->armaTextoLargoFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, 80);
                    break;
                case 'Web':
                    $txtFiltro.=$this->armaWebFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, 80);
                    break;
            }
        }
        return $txtFiltro;
	...
	protected function armaListaUsuarios($id_carac,$titulo,$contenido) {
                $usuarios = new LoginwebuserBSN();
		print "<td>$titulo<br />";
                $usuarios->comboUsuariosMultiple($contenido, 'l_'.$id_carac);
//		if($comentario=='Si') {
//			$campo='com_'.$id_carac;
//			$this->armaCampo($campo,$contcoment);
//		}
...
//		print "<table width='100%'>";
		foreach ($arrayCarac as $elemCarac) {
			...
			switch ($elemCarac['tipo']) {
				case 'CheckBox':
                    echo $this->armaCheckboxFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, '', '', 80);
					break;
				case 'Lista':
                    echo $this->armaListaFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $elemCarac['lista'], $valor, 80);
					break;
				case 'Numerico':
					$compara=$caracBSN->cargaComparacionCarac($elemCarac['id_carac']);
                    echo $this->armaNumericoFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $elemCarac['maximo'], '', $valor, '', 80, $compara);
					break;
				case 'Texto':
                    echo $this->armaTextoFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, 100);
					break;
				case 'Texto Largo':
                    echo $this->armaTextoLargoFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, 80);
					break;
				case 'Web':
                    echo $this->armaWebFiltro($elemCarac['id_carac'], $elemCarac['titulo'], $valor, 80);
					break;
			}
			//			print "</td>\n";
			//                        print "</tr>\n";
...
//		print "</tr></table>\n";
	...
	public function armaTextoFiltroCRM($array,$tipo_prop) {
		$caracBSN = new CaracteristicaBSN();
		$tipocarac = new Tipo_caracBSN();
		$carac = new Caracteristica();
		//		echo "Prop DPVW -> $tipo_prop<br>";
		...
		foreach ($arrayCarac as $elemCarac) {
			if(key_exists($elemCarac['id_carac'],$array)===true) {
				$valor=$array[$elemCarac['id_carac']][1];
				$txtcarac.=($elemCarac['titulo'].": ".$valor."|");
			}
		}
		return $txtcarac;
	}
	...
	protected function armaListaFiltro($id_carac,$titulo,$lista,$contenido,$ancho) {
		...
		$campo='l_'.$id_carac;
		$ancho-=20;
        $txt.=("<div class='titu_filtro' id=\"titu_" . $id_carac . "\" onClick=\"cambioVistas('" . $id_carac . "');\">$titulo</div>");
        $txt.=( "	<div id=\"" . $id_carac . "\" name=\"" . $id_carac . "\">\n");
		
        $txt.=( "<table width='100%'>\n");
        //		$txt.=( "<td class='cd_celda_texto'>$titulo</td></tr>";
        $txt.=( "<tr><td width='" . $ancho . "%'>");
        $txt.=( "<select name='" . $campo . "' id='" . $campo . "' class='campos_btn'>\n");
		for ($pos=0;$pos<sizeof($array);$pos++) {
            $txt.=( "<option value='" . $array[$pos] . "'");
			if ($array[$pos]==$contenido) {
                $txt.=( " SELECTED ");
			}
            $txt.=( ">" . $array[$pos] . "</option>\n");
		}
        $txt.=( "</select>\n");
        $txt.=( "</td></tr></table>\n");
        $txt.=( "</div>\n");
	...
	protected function armaNumericoFiltro($id_carac,$titulo,$maximo,$comentario,$contenido,$contcoment,$ancho,$compara) {
        ...
        //		$txt.=( "<td class='cd_celda_texto' width='15%'>$titulo ";
		$ancho-=20;
        $txt.=( "<tr><td width='$ancho%'>");
		if($maximo>0) {
			$campo='n_'.$id_carac;
	...
	protected function armaCampoEntre($campo,$contenido) {
        ...
        $txt.=( "function armaCompara(destino){\n");
        $txt.=( "     val1=document.getElementById('a1'+destino).value;\n");
        $txt.=( "    	val2=document.getElementById('a2'+destino).value;\n");
        $txt.=( "     if(isNaN(val1) || isNaN(val2) || parseInt(val1) > parseInt(val2)){\n");
        $txt.=( "         alert(\"Debe ingresar valores numericos y el segundo debe ser mayor al primero\");\n");
        $txt.=( "     	document.getElementById('a1'+destino).focus();\n");
        $txt.=( "     }else{\n");
        $txt.=( "     	document.getElementById(destino).value=val1+' and '+val2;\n");
        $txt.=( "     }\n");
        $txt.=( "}\n");
        $txt.=( "</script>\n");
        ...
        $txt.=( " Desde ");
        $txt.=( "<input class='campos' type='text' name='a1" . $campo . "' id='a1" . $campo . "' value='" . $cont1 . "' maxlength='10' size='20'><br>");
        $txt.=( " Hasta ");
        $txt.=( "<input class='campos' type='text' name='a2" . $campo . "' id='a2" . $campo . "' value='" . $cont2 . "' maxlength='10' size='20' onblur='javascript:armaCompara(\"$campo\");'>");
        $txt.=( "<input class='campos' type='hidden' name='$campo' id='$campo' value='" . $contenido . "' maxlength='10' size='20'>");
        return $txt;
	}
	...
	protected function armaCampoFiltro($campo,$contenido) {
        ...
        $txt.=( "<input class='campos' type='text' name='$campo' id='$campo' value='" . $contenido . "' maxlength='200' size='40'>");
        $txt.=( "</td>\n");
        $txt.=( "</tr>\n");
        return $txt;
	}
	...
	protected function armaCheckboxFiltro($id_carac,$titulo,$contenido,$comentario,$contcoment,$ancho) {
        ...
        $txt.=( "<tr><td width='$ancho%'>");
		$campo='c_'.$id_carac;
        $txt.=( "<input type='checkbox' id='" . $campo . "' name='" . $campo . "'");
		if ($contenido=='on') {
            $txt.=( " checked ");
		}
        $txt.=( ">\n");
	...
	protected function armaTextoFiltro($id_carac,$titulo,$contenido,$ancho) {
        ...
        $txt.=( "	<div id=\"" . $id_carac . "\" name=\"" . $id_carac . "\">\n");
        $txt.=( "<table width='100%'>");
        //		$txt.=( "<td class='cd_celda_texto' width='15%'>$titulo</td></tr>";
		$ancho-=20;
        $txt.=( "<tr><td width='$ancho%'>");
		$campo='t_'.$id_carac;
		$this->armaCampo($campo,$contenido);
	...
	protected function armaTextoLargoFiltro($id_carac,$titulo,$contenido,$ancho) {
        ...
        $txt.=( "<tr><td width='$ancho%'>");
		$campo='b_'.$id_carac;
        $txt.=( "<textarea cols='40' rows='4' class='campos_area' name='$campo' id='$campo'>" . $contenido . "</textarea>");
        $txt.=( "</td></tr></table>\n");
        $txt.=( "</div>\n");
        return $txt;
	}
