 ...
 */
	public function comboTipoProp($valor=0,$opcion=0,$campo="id_tipo_prop",$class="campos_btn",$div='',$subtipo='',$divdatos=''){
		...
		if($div!=''){
			if($div=='filtro'){
				...
				$onchange="comboSubtipo_prop('".$campo."','".$div."','".$subtipo."','subtipo_prop');validaTipo();";
//				print "onchange=\"javascript: comboSubtipo_prop('".$campo."','".$div."','".$subtipo."','subtipo_prop');\"";
			}
		}
		if($divdatos!=''){
			$onchange.=" llenaDatosprop('".$campo."','".$divdatos."');";
		}
		if($onchange!=''){
				print "onchange=\"javascript: $onchange\"";
		}
		print ">\n";
		switch ($opcion) {
			case 1:
		...
		for ($pos=0;$pos<sizeof($tipoProp);$pos++){
			if($opcion==3 || $gtp->perteneceGrupo($tipoProp[$pos]['id_tipo_prop'],$gpf) || $gpf=='GRAL' || $gpf=='SUPERUSER' || $gpf=='LECTURA' || $gpf=='admin' || $gpf == 'GRAL' || strtoupper($gpf) == 'STAFF' || strtoupper($gpf) == 'GERENCIA'){
 ...
 * @param str $class -> clase de la css con la cual se presenta el dato
 */
	public function comboSubtipoProp($valor=0,$id_tipo_prop=0,$opcion=0,$campo="subtipo_prop",$class="campos_btn",$div=''){
		if($id_tipo_prop!=0){
		...
		switch ($opcion) {
			...
			case 2:
				...
				print ">Seleccione un tipo de propiedad</option>\n";
				break;
		}
	...
	public function checkboxTipoProp($valor,$opcion=0){
		...
		foreach ($tipoProp as $funcion){
			...
			print " value='".$funcion['tipo_prop']."' />&nbsp;&nbsp;".$funcion['tipo_prop']."</td>\n";
		}
		print "</tr></table>\n";
...
//			if($post[$campo]=="on"){
	...
	public function grabaCaracteristica_TipoProp($id_carac,$arrayTPAsig){
        ...
        /**
         * Arma un array simple donde la clave del mismo es el ID del tipo de propiedad y e contenido la Descripcion
         */
        public function armaArrayTipoProp(){
            $arrayRet=array();
            $arrDatos=  $this->cargaColeccionForm();
            foreach ($arrDatos as $dato){
                $arrayRet[$dato['id_tipo_prop']]=$dato['tipo_prop'];
}
            return $arrayRet;
        }
}
