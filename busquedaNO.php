	...
	//print_r($_GET);
		...
		if($_GET['opcUbica'] != ''){
			for($i=0; $i < count($zona); $i++) {
				if($zona[$i]['nombre_ubicacion'] == str_replace("-", " ", $_GET['opcUbica'])){
...
*/        
        ...
        ?>
  <div id="resultado">
    <div id="fotoResul" border="0"><a href="<?php echo strtolower(sanear_string($txtTipo));?>_<?php echo strtolower(sanear_string($prop[$i]['operacion']));?>_<?php echo strtolower(sanear_string(substr(busca_valor($prop[$i]['id_prop'], 257, $carac), 0, 100)));?>_<?php echo $id_prop; ?>">
      <?php if($fotos[0]['foto'] != "") { ?>
      <img src="http://abm.okeefe.com.ar/fotos_th/<?php echo $fotos[0]['foto']; ?>" width="160" height="107" border="0" />
      <?php }else { ?>
      <img src="images/noDisponible.gif" width="160" height="107" />
      <?php }?>
    </a></div>
    <div id="contenidoResul" style="float:right; width:830px; height:107px; margin-left:5px;">
      <div id="cabezaResul">
        <div id="zonaResul"><a href="<?php echo strtolower(sanear_string($txtTipo));?>_<?php echo strtolower(sanear_string($prop[$i]['operacion']));?>_<?php echo strtolower(sanear_string(substr(busca_valor($prop[$i]['id_prop'], 257, $carac), 0, 100)));?>_<?php echo $id_prop; ?>"><?php echo substr(busca_valor($prop[$i]['id_prop'], 257, $carac), 0, 100);?></a></div>
...
<span class="tituResult">Superficie:</span> <span class="caracResul">
        ...
        <div id="descripResul"><a href="<?php echo strtolower(sanear_string($txtTipo));?>_<?php echo strtolower(sanear_string($prop[$i]['operacion']));?>_<?php echo strtolower(sanear_string(substr(busca_valor($prop[$i]['id_prop'], 257, $carac), 0, 100)));?>_<?php echo $id_prop; ?>">
        <div style="height:71px; max-height:71px; overflow:hidden;"><span class="tituResult">Descripción:</span><br />
          ...
          <?php
            ...
            <div>
          <div id="vermasResul"><a href="<?php echo strtolower(sanear_string($txtTipo));?>_<?php echo strtolower(sanear_string($prop[$i]['operacion']));?>_<?php echo strtolower(sanear_string(substr(busca_valor($prop[$i]['id_prop'], 257, $carac), 0, 100)));?>_<?php echo $id_prop; ?>" style="color:#FFF; text-decoration:none">Más detalles</a></div>
          </div>
          
        </div>
        <div id="datos2Resul">
        <?php if($prop[$i]['id_emp'] != 0){ ?>
        	<div id="verEmpre"><a href="detalleEmprendimiento.php?i=<?php echo $prop[$i]['id_emp'];?>" style="color:#FFF;">Emprendimiento</a></div>
        <?php }else{ ?>
        	<div style="height:18px;margin-bottom:4px;"></div>
        <?php } ?>
          <div class="consultar" style="margin-bottom:4px;"><a href="form_recomendar.php?id=<?php echo str_repeat("0", 5-strlen(strval($id_prop))) . $id_prop;?>&TB_iframe=true&height=250&width=300&modal=false" class="thickbox" rel="nofollow">&gt; Recomendar</a></div>
          <div class="consultar"><a href="form_consulta_prop.php?id=<?php echo str_repeat("0", 5-strlen(strval($id_prop))) . $id_prop;?>&t=<?php echo $prop[$i]['id_tipo_prop']; ?>&TB_iframe=true&height=250&width=800&modal=false" class="thickbox" rel="nofollow">&gt; Consultar</a></div>
