...
//		$(this).css({width: '90%', height: '90%'});
	...
	img.attr('src', 'digital/large/' +  page + '.jpg');
}
...
//    $('.chk_box_der').css({top:'-289px',left:'70px'});
    var top=$('div.gradient').height();
    if(alto < top){
        $('.chk_container').css({top:top,left:'0px',width:'100%',height:'40px'});
    }else{
        top=($('div.magazine-viewport').height()-$('div.gradient').height())/3+$('div.gradient').height();      
        $('.chk_container').css({top:top,left:'0px',width:'100%',height:'40px'});
    }
    var posIzq=$(window).width()/2-70;
    var posDer=$(window).width()/2+50;
...
function marcaSelector(clicked,turned){
    ...
    if(clicked!=0){
        ...
        for(var x=0; x < selecciones.length; x++){
            if(turned%2==0){
                if(selecciones[x]==turned){
                        document.getElementById('chk_box_izq').checked=true;
                        marca=1;
                }
                if(selecciones[x]==turned+1){
                        document.getElementById('chk_box_der').checked=true;
                        marcad=1;
                }
            }else{
                if(selecciones[x]==turned-1){
        ...
        if(marcad==0){
              document.getElementById('chk_box_der').checked=false;
        }
        if(marca==0){
               document.getElementById('chk_box_izq').checked=false;
        }
...
function recuperaListaSelecciones(){
    ...
    if(selecciones.length!=0){
        retorno='Solicito informacion respecto a publicaciones de página/s '+ selecciones.join();
