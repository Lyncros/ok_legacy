...
function bannerRotator1(selector, scrollTime, pauseTime){
  
  $(selector+" li:first").css("display", "block"); //show the first list item
  var count = $(selector+" li").size(); //get total number of list items
  if(count > 1){ //dont do anything if there is only one list item.  
	
	  if(scrollTime1 == null){ var scrollTime1=500; } //default scroll time (length of transition)
    if(pauseTime1== null){ var pauseTime1=5000; } //default pause time (how long to hold the image between transitions)
