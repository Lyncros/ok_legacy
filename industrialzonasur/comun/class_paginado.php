...
class class_paginado{
	...
	/** ejecutar el SQL original para contar la cantidad de registros, de otra manera se modifica el query para contar la cantidad de registros de una manera más rapida, aunque a veces falla al modificarlo */
	...
	function mostrar_paginado(){
		...
		if($this->total_registros <= $this->registros_por_pagina){
			if($this->mostrarTotalRegistros){
				...
				echo "<span class='rotuloTotalRegistros'>".$this->str_total.": ".$this->total_registros." ".($this->total_registros > 1 ? $this->str_registros : $this->str_registro)."</span>";
				echo "</div>";
			}
			return;
		}
		...
		if(($this->registro - $this->registros_por_pagina)>=0){
			...
			if ($this->mostrar_ultima_primera_pagina and $this->registro-$this->registros_por_pagina > 0) {
				echo "<a href='$this->parent_self?".$this->nombre_var_registro."=0$qs' class='link_pri'>".$this->primera."</a> ";
			}
			...
			if($this->mostrar_anterior_sigiente_inabilitado) echo "<span class='ant_desact'>".$this->anterior_des."</span> ";
		}
		...
		if(($this->registro + $this->registros_por_pagina) < $this->total_registros){
			...
			if($this->mostrar_anterior_sigiente_inabilitado) echo "<span class='sig_desact'>".$this->siguiente_des."</span>";
		}
		
		//link ultima pagina
		if ($this->mostrar_ultima_primera_pagina and ( ($this->registro+$this->registros_por_pagina) < $this->total_registros) ) {
			echo "<a href='$this->parent_self?".$this->nombre_var_registro."=".($this->registros_por_pagina*(ceil($this->total_registros/$this->registros_por_pagina)-1))."$qs' class='link_ult'>".$this->ultima."</a> ";
		}

		if($this->mostrarTotalRegistros) echo "&nbsp;&nbsp; <span class='rotuloTotalRegistros'>".$this->str_total.": ".$this->total_registros." ".($this->total_registros > 1 ? $this->str_registros : $this->str_registro)."</span>";
