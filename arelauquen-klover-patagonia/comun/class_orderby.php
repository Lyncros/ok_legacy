...
class class_orderby{
	 ...
	 * Constructor
	 *
	 * @param string $orderByPorDefecto Order by por defecto
	 * @param string $camposOrder Campos por los que se puede ordenar separados por |
	 * @param string $variableOrderBy Nombre de la variable que pasa por GET para ordenar
	 */
	...
	private function leerYguardarOrderBy(){
		if (isset($_GET[$this->variableOrderBy]) and trim($_GET[$this->variableOrderBy])!="") {
			...
			if(array_key_exists($i-1, $this->arrayCamposOrder)){
				...
				//se mandó un order que no existe!
	...
	/**
	 * Obtiene el order by para la consulta SQL. Usar: "...ORDER BY ".$o->getOrderBy()
	 */
	 ...
	 *
	 * @param String $txt_campo Texto del link
	 * @param String $campo Campo a ordenar. Debe existir en la lista de campos definida en el constructor
	 * @return String Codigo HTML
	 */
	public function linkOrderBy($txt_campo, $campo){
		$keyCampo = array_search($campo, $this->arrayCamposOrder);
		if($keyCampo===false) throw new Exception("El campo $campo no está definido en el constructor de la clase!");
