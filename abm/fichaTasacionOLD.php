...
if ($prop ['piso'] != '') {
	$direccionProp .= " Piso ". $prop ['piso'];
}
if ($prop ['dpto'] != '') {
	$direccionProp .= " Depto. " . $prop ['dpto'];
}
if ($prop ['entre1'] != '') {
	$direccionProp .= '<br />Entre ' . $prop ['entre1'] . ' y ' . $prop ['entre2'];
}
if ($prop ['cantamb'] != 0) {
...
$cabezal .= '<tr style="text-align:center;"><td>amb</td><td>ID ' . str_repeat ( "0", 5 - strlen ( strval ( $id_prop ) ) ) . $id_prop . '</td></tr>';
$cabezal .= '</table>';
...
$pdf->MultiCell(85, 0.5, "Comentarios:", 0, 'L', false, 0, 11, ($y_inicio + 0.5));
$pdf->SetTextColor(60, 60, 60);
