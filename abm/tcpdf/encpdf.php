...
//        $this->SetFontSize(9);
    ...
    function Linea($titulo="-",$texto="", $col=1) {
    	if($col==1){
    		$x=10;
    	}else{
    		$x=108;
        ...
        $wtxt=$this->GetStringWidth($texto);
        $htxt = $this->getStringHeight(45, $titulo);
        if($htxt > 4){
            $h=$htxt+1;
        }else{
            $h=0;
        ...
        $this->MultiCell(45,$h,trim($titulo),'BR','L', 1, 0, '', '', true, 0, false, true, 0);
        //}else{
        ...
        if ($wtxt <= 40) {
            ...
            if($texto == "on") {
                ...
                $this->Image('images/tilde.gif',$x+48, $this->GetY(), 3, 3.2,'GIF','');
            }else {
                $this->Cell(42,$h,trim($texto),'B',0,'L',$fill);
            }
            $this->Ln();
        } else {
            $this->MultiCell(42,$h,trim($texto),'B','L', 1, 1, '', '', true, 0, false, true, 0, 'T');
        }
    }
