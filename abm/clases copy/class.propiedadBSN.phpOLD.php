...
//		$localclass=$this->getClase().'PGDAO';
    ...
    public function cargaColeccionFiltroBuscador($zona,$localidad,$tipo_prop,$operacion,$id_emp,$in,$pagina=1) {
//		$localclass=$this->getClase().'PGDAO';
        $config= new CargaConfiguracion();
        $registros=$config->leeParametro('regprod_adm');
        $array=array();
        ...
        $arrayTabla=$this->mapa->objTOtabla($prop);
        $datoDB = new PropiedadPGDAO($arrayTabla);
        $array = $this->leeDBArray($datoDB->coleccionByFiltroBuscador($zona,$localidad,$tipo_prop,$operacion,$id_emp,$in,$pagina,$registros));
        $arrayform=array();
        foreach ($array as $registro) {
            $arrayform[]=$this->mapa->tablaTOform($registro);
        }
