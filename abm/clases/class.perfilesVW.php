...
//		PerfilesVW::creaPerfiles();
		PerfilesVW::creaObjeto();
		if ($_perfil instanceof Perfiles ) {
//			PerfilesVW::seteaPerfiles( $_perfil);
			PerfilesVW::seteaVW( $_perfil);
		}
		if (!is_numeric ( $_perfil)) {
			if ($_perfil != '') {
				PerfilesVW::cargaVW ( $_perfil );
			}
		}
		PerfilesVW::cargaDefinicionForm();
	}
...
//		$menu->dibujaMenu ( 'lista', 'opcion' );
		...
		if (sizeof ( $arrayEven ) == 0) {
			...
			foreach ( $arrayEven as $Even ) {
				...
				print "    <a href='javascript:envia(\"lista\",822,\"" . $Even ['perfil'] . "\");' border='0'>";
				print "       <img src='images/building_edit.png' alt='Editar' title='Editar' border=0></a></td>";
				print "	 <td  align='center' width='25' class='row" . $fila . "'>";
				print "    <a href='javascript:envia(\"lista\",823,\"" . $Even ['perfil'] . "\");' border=0 onclick=\"return confirm('Esta seguro que quiere eliminar este registro?');\">";
				print "       <img src='images/delete.png' alt='Borrar' title='Borrar' border=0></a>";
				print "  </td>\n";
				print "	 <td align='center' width='25' class='row" . $fila . "'>";
				print "    <a href='javascript:envia(\"lista\",824,\"" . $Even ['perfil'] . "\");' border='0'>";
