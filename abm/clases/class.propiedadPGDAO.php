...
class PropiedadPGDAO extends PGDAO {
    ...
    protected $COLECCIONBASE = "SELECT p.id_prop,p.id_ubica,p.calle,p.entre1,p.entre2,p.nro,p.descripcion,p.id_tipo_prop,p.subtipo_prop,
        p.intermediacion,p.id_inmo,p.operacion,p.comentario,p.video,p.piso,p.dpto,p.id_cliente,p.goglat,p.goglong,p.activa,p.id_sucursal,
        p.id_emp,p.nomedif, p.plano1, p.plano2, p.plano3, p.id_parcela, p.id_comercial, p.compartir,p.publicaprecio,p.destacado,
        p.oportunidad,p.publicadir FROM #dbName#.propiedad as p ";
    
    protected $JOIN = " INNER JOIN (
            SELECT q.id_prop,q.operacion FROM #dbName#.propiedad_operacion as q inner join ( select id_prop,max(fecha) as fecha from  #dbName#.propiedad_operacion group by id_prop) as j on (q.id_prop=j.id_prop and q.fecha=j.fecha)) as o ON p.id_prop=o.id_prop ";
    ...
    protected $JOINBUSCADOR = " LEFT JOIN #dbName#.ubicacionpropiedad as z ON p.id_ubica = z.id_ubica INNER JOIN #dbName#.tipoprop as t ON p.id_tipo_prop = t.id_tipo_prop ";
...
//        $INOPER=" (select id_prop from #dbName#.propiedad_operacion where fecha > '$fecha' AND operacion in ($operacion) group by id_prop) ";
    ...
    public function coleccionZonasActivas($operacion = '', $activas = -1, $id_ubicaciones = 0) {
        $parametro = func_get_args();
        $sqlStr = "SELECT distinct id_ubica,operacion,id_tipo_prop,activa,count(id_ubica) as id_prop " .
                "FROM #dbName#.propiedad ";
                
        $group_by = " GROUP BY id_ubica,operacion,id_tipo_prop,activa ";
        $order = " ORDER BY id_ubica,operacion,id_tipo_prop,activa ";
        $where = '';
        $having = '';

        if ($id_ubicaciones) {
            $where = ' WHERE id_ubica IN (' . $id_ubicaciones . ') ';
        }
        ...
        $resultado = $this->execSql($sqlStr . $where . $group_by . $having . $order, $parametro);
        if (!$resultado) {
