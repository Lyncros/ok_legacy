...
class DatospropVW {
	...
	private function array_column($array, $col) {
		$colData = array();
	...
	public function cargaDatosDatospropDiv($id_prop,$tipo_prop,$div='datosprop') {
		...
		$caracIds = $this->array_column($arrayCarac, 0);
		$caracteristicas = $caracBSN->cargaColeccionById($caracIds);

		$datosPropBSN = new DatospropBSN();
		$datosPropByCaracID = $datosPropBSN->getColeccionByPropCaracID($id_prop, $caracIds);
		...
		foreach ($arrayCarac as $elemCarac) {
			...
			$id_carac = $elemCarac[0];
			$carac = $caracteristicas[$id_carac];

            if (array_key_exists($id_carac, $datosPropByCaracID)) {
				$arrayDatos = $datosPropByCaracID[$id_carac];
		...
		foreach ($arrayCarac as $elemCarac) {
			...
			switch ($elemCarac['tipo']) {
				...
				case 'Numerico':
					...
					break;
				case 'Texto':
					$this->armaTextoFiltro($elemCarac['id_carac'],$elemCarac['titulo'],$valor,100);
					break;
				case 'Texto Largo':
...
//		print "</tr></table>\n";
	...
	protected function armaListaFiltro($id_carac,$titulo,$lista,$contenido,$ancho) {
		...
		$campo='l_'.$id_carac;
		$ancho-=20;
		print "<div class='titu_filtro' id=\"titu_".$id_carac."\" onClick=\"cambioVistas('".$id_carac."');\">$titulo</div>";
		print "	<div id=\"".$id_carac."\" name=\"".$id_carac."\">\n";
