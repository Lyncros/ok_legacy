...
//					$datos->setComentario($arrayCarac[$id_carac]['comentario']);
					if(is_array($cont )){
						$datos->setId_carac($id_carac);
						$datos->setContenido(implode(",",$arrayCarac[$id_carac]['contenido']));                        
					}else{
						$datos->setId_carac($id_carac);
						$datos->setContenido($arrayCarac[$id_carac]['contenido']);
					}
	...
	public function getColeccionByPropCaracID($id_prop, $ids_carac) {
		$dao = new DatospropPGDAO();
		$data = $dao->coleccionPropCaracBulk($id_prop, $ids_carac);
		$dbArray = $this->leeDBArray($data);
		$caracById = array();
        foreach ($dbArray as $registro) {
            $id = $registro['id_carac'];
            $caracById[$id] = $this->mapa->tablaTOform($registro);
        }
        return $caracById;
	}
