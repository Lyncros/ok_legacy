...
class DatosempVW extends VW {
	...
	public function __construct($_datosemp=0){
		DatosempVW::creaObjeto();
		if($_datosemp instanceof Datosemp )	{
			DatosempVW::seteaVW($_datosemp);
		}
		if (is_numeric($_datosemp)){
			if($_datosemp!=0){
				DatosempVW::cargaVW($_datosemp);
			}
		}
		DatosempVW::cargaDefinicionForm();
	}
	...
	public function cargaDatosVW($id_emp,$pagorig){
	...
	public function cargaDatosVW2($id_emp,$pagorig){
	...
	protected function armaWeb($id_carac,$titulo,$contenido,$ancho){
		...
		$rFCKeditor->config['width'] = 750;
		$rFCKeditor->config['height'] = 400;

		//$rFCKeditor->Value		= $contenido ;
		//$rFCKeditor->Height = 400;
		//$rFCKeditor->Width = 700;
		//$rFCKeditor->resize(700,400,true);
		print "<td class='cd_celda_texto' width='15%'>$titulo</td>";
		$ancho-=130;
		print "<td width='$ancho'>";
		//$rFCKeditor->Create() ;
	 ...
	 *
	 */
	public function vistaTablaVW($id){

		$emp=new EmprendimientoBSN($id);
		$fila=0;
		print "<span class='pg_titulo'>Listado de Caracteristicas del emprendimiento de </span><br>\n";
		...
		if(sizeof($arrayEven)==0){
			...
			foreach ($arrayEven as $Even){
				...
				$renglon.= "    <a href='javascript:envia(\"lista\",343,".$Even['id_emp_carac'].");' border=0  onclick=\"return confirm('Esta seguro que quiere eliminar este registro?');\">";
				$renglon.= "       <img src='images/delete.png' alt='Borrar' title='Borrar' border=0></a></td>\n";

				$renglon.= "	 <td class='row".$fila."' width='16'>";
				$renglon.= "    <a href='javascript:envia(\"lista\",344,".$Even['id_emp_carac'].");' border='0'>";
				$renglon.= "       <img src='images/camera_edit.png' alt='Fotografias' title='Fotografías' border=0></a></td>\n";
				$renglon.= "	 <td class='row".$fila."'>";
				if($Even['activa']==1){
					$renglon.= "    <a href='javascript:envia(\"lista\",346,".$Even['id_emp_carac'].");' border=0>";
					$renglon.= "       <img src='images/web_no.png' alt='No Publicar en Web' title='No Publicar en Web' border=0></a>";
				}else{
					$renglon.= "    <a href='javascript:envia(\"lista\",345,".$Even['id_emp_carac'].");' border=0>";
