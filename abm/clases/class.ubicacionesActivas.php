...
class UbicacionesActivas {
    ...
    private function __construct($id_padre = 0) {
        UbicacionesActivas::cargaUbicacionesActivas($id_padre);
    }

    static public function getInstance($id_padre = 0) {
        if (is_null(self::$_instance)) {
            self::$_instance = new self($id_padre);
        }
    ...
    protected function cargaUbicacionesActivas($id_padre = 0) {
        $propBSN = new PropiedadBSN(0, '');
        $tpropBSN = new Tipo_propBSN(0, '');
        $arrayTP = $tpropBSN->cargaColeccionForm();
        $arrayZA = $propBSN->cargaColeccionZonasActivas($id_padre);
