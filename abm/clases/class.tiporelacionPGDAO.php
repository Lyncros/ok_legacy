...
class TiporelacionPGDAO extends PGDAO {
	...
	protected $COLECCIONBYPARTE="SELECT id_tiporel,tiporelacion,relacion,grado,observacion FROM #dbName#.tiporelacion WHERE tiporelacion like '%#tiporelacion#%' ORDER BY tiporelacion,relacion,grado";	
	...
	public function coleccionByComponente($componente,$_parte){
        	$COLECCIONBYCOMPONENTE="SELECT id_tiporel,tiporelacion,relacion,grado,observacion FROM #dbName#.tiporelacion WHERE substring(tiporelacion,$_parte,1)='$componente' ORDER BY tiporelacion,relacion,grado";	
		$parametro = func_get_args();
		$resultado=$this->execSql($COLECCIONBYCOMPONENTE,$parametro);
		if (!$resultado){
			$this->onError($COD_COLLECION,"RELACIONES POR PARTE DE LA RELACION");
		}
		return $resultado;
	}		
