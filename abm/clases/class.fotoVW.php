...
class FotoVW extends VW {
	...
	public function vistaTablaVW($id_prop=0){
		...
		if($id_prop==0 || is_nan($id_prop)) {
			...
			print "     <td class='cd_lista_titulo' width='100'>Publica en la web</td>\n";
			print "	  </tr>\n";
			...
			if(sizeof($arrayFoto)==0){
				...
				foreach ($arrayFoto as $foto){
					...
					if ($foto['publica_web']) {
						$iconPublicaWeb = 'web.png';
						$txtAction = 'Ocultar';
						$jsAction = 'ocultar';
					} else {
						$iconPublicaWeb = 'web_no.png';
						$txtAction = 'Mostrar';
						$jsAction = 'mostrar';
					}

					print "	 <td class='row".$fila."'>";
					print "    <img src='images/".$iconPublicaWeb."' alt='Bajar' border=0>\n";
					print "    <a href='javascript:".$jsAction."($id_prop,".$foto['id_foto'].");' border=0>";
					print "      ".$txtAction;
					print "    </a>\n";
					print "  </td>\n";
					print "</tr>\n";
