...
class CaracteristicaPGDAO extends PGDAO {
	...
	protected $COLECCIONBYID = "SELECT id_carac,id_tipo_carac,titulo, tipo,maximo,comentario,lista,orden,publica,tasacion 
		FROM #dbName#.caracteristica WHERE id_carac in (#ids_carac#)";
...
//		$parametro = func_get_args ();
	...
	public function coleccionById($ids) {
		$ids_carac = implode($ids, ',');
		$this->seteaDato(array('ids_carac' => $ids_carac));
		$resultado = $this->execSql($this->COLECCIONBYID, $ids_carac);
		if (!$resultado){
			$this->onError("COD_COLLECION","CARACTERISTICAS bulk por lista de IDs");
		}
		return $resultado;
	}
