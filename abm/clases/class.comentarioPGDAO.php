...
class ComentarioPGDAO extends PGDAO {
    ...
    protected $COLECCION = "SELECT id_com,DATE_FORMAT(fecha,'%d-%m-%Y') as fecha,id_user,id_prop,visible,comentario  FROM #dbName#.comentario WHERE visible=1 ORDER BY DATE_FORMAT(fecha,'%Y-%m-%d') DESC";
...
// order by id_prop,fecha,concepto;        
    ...
    public function coleccionComentarios($id_prop,$visible=1, $concepto) {
        ...
        if ($concepto != 0 && ($concepto != '' && is_array($concepto))) {
            ...
            $orden = " order by DATE_FORMAT(fecha,'%Y-%m-%d') DESC, id_prop,concepto";        
