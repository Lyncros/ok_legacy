...
//		FotoempVW::creaFotoemp();
		FotoempVW::creaObjeto();
		if($_foto instanceof Fotoemp )	{
//			FotoempVW::seteaFotoemp($_foto);
			FotoempVW::seteaVW($_foto);
		}
		if (is_numeric($_foto)){
			if($_foto!=0){
//				FotoempVW::cargaFotoemp($_foto);
				FotoempVW::cargaVW($_foto);
			}
		}
		FotoempVW::cargaDefinicionForm();
 ...
 *
 */
	public function vistaTablaVW($id_emp_carac=0){
		...
		if($id_emp_carac==0 || is_nan($id_emp_carac)) {
			...
			print "}\n";
			print "function cargaFotos(id_emp_carac){\n";
			print "   window.open('carga_FotoEmp.php?o='+id_emp_carac+'&f=0', 'ventanaFotoEmp', 'menubar=1,resizable=1,width=950,height=550');\n";
			print "}\n";
			print "</script>\n";
			
			$arrayTools=array(array('Nuevo','images/camera_edit.png','cargaFotos('.$id_emp_carac.')'));
			$menu=new Menu();
			print "<span class='pg_titulo'>Carga de Fotos para ".$carac." del emprendimiento ".$emp."</span><br><br>\n";
...
//			$menu->dibujaMenu('lista');
			...
			if(sizeof($arrayFotoemp)==0){
				...
				foreach ($arrayFotoemp as $foto){
					...
					print "    <a href='javascript:envia(\"lista\",3443,".$foto['id_foto'].");' border=0 onclick=\"return confirm('Esta seguro que quiere eliminar este registro?');\">";
					print "       <img src='images/delete.png' alt='Borrar' title='Borrar' border=0></a></td>";
					print "	 <td class='row".$fila."' width='16'>";
					print "    <a href='javascript:envia(\"lista\",3444,".$foto['id_foto'].");' border='0'>";
					print "       <img src='images/up.png' alt='Subir' border=0></a></td>";
					print "	 <td class='row".$fila."' width='16'>";
					print "    <a href='javascript:envia(\"lista\",3445,".$foto['id_foto'].");' border=0>";
	...
	public function leeDatosFotoempVW_ORIG(){
		...
		if($_FILES['foto']['type']=='image/jpeg' || $_FILES['foto']['type']=='image/gif' || $_FILES['foto']['type']=='image/png'){
			...
			$nom=remover_acentos($_FILES['foto']['name']);
			$nombre='CE_'.$_POST['id_emp_carac'].'_'.substr($nom,0,strlen($nom)-4);
			if($fotoup->uploaded){
				$fotoup->image_resize 			= true;
				$fotoup->image_ratio_y 			= true;
